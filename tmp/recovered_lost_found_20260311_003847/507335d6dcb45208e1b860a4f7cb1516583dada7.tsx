...
├── menu-upload.md     — Menu upload parsing + OCR flow
...
Phase 2 — Backend Wiring (CURRENT)
  ...
  ✓ Deterministic menu parser Edge Function (position-based PDF extraction + text parsing, no LLM fallback)
...
- **Menu parser PDF fallback**: When PDF extraction fails, do not decode raw PDF bytes as text; return empty parsed output with review guidance instead.

- **Menu parser corrupted text guard**: Treat symbol-heavy or corrupted extracted text as invalid and return empty parsed output. In admin upload flow, stay on upload with a clear error when parser returns zero items.

- **Menu image OCR handoff**: Image uploads must send OCR text as `rawText` to `parse-menu`. If image inputs reach the Edge Function without `rawText`, return a retry/manual-review message instead of proceeding with empty parsing.
