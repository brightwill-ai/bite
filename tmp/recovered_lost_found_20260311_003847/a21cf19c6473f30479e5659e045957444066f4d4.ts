...
- Admin menu upload uses local PDF extraction (`/api/extract-pdf`) with position-based line reconstruction and detached-price reordering, then falls back to `parse-menu` server extraction if local extraction fails.
...
- `parse-menu` is deterministic-only: text/PDF parsing with confidence gating and `needs_review` fallback when confidence is low.
...
- Image uploads are unsupported in deterministic mode; parser returns an empty result with review guidance, so upload PDF/TXT sources instead.
