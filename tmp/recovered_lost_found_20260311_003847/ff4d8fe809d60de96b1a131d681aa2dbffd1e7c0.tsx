...
- Mirror admin/server PDF extraction settings exactly: `pdfjs-dist@5.5.207` with `cMapUrl`, `cMapPacked`, `standardFontDataUrl`, and `useSystemFonts` to avoid custom-font garbling
...
- Admin upload page uses `/api/extract-image` (Tesseract OCR) for `.png/.jpg/.jpeg/.webp` and sends OCR output as `rawText`
...
- Parser strategy:
  - deterministic parser over extracted text (PDF/TXT and OCR text from supported images)
  ...
  - if image input reaches `parse-menu` without `rawText`, return empty parse output with retry guidance
