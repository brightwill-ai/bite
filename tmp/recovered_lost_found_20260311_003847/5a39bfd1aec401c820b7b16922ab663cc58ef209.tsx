...
async function extractImageTextViaApi(file: File): Promise<string> {
  const formData = new FormData()
  formData.append('file', file)
...
export default function MenuUploadPage() {
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (isPdf) {
          ...
          const extractedImageText = await extractImageTextViaApi(file)
          rawText = extractedImageText.trim() || undefined
        } else {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isSupportedMenuFile(file)) {
      setError('Only PDF, image, and TXT files are supported.')
      return
    }
