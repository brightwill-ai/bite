...
- Failing hard on local PDF extraction; `/api/extract-pdf` now uses position-based ordering and price-line repair, but upload must still stay resilient by falling back to `parse-menu` edge extraction when local extraction fails.
